...
export default {
	name: 'OperatorList'
	, components: {OperatorItem, Empty, Search, Excel, Pagination, BackgroundModal}
	...
	,methods: {
		getData: async function(){
			...
			try{
				...
				const result = await this.$request.init({
					method: 'get'
					,url: 'management/getOperatorList'
					,data: this.search
				})
		...
		,toDetail: function (item){
			this.$storage.push({ name: 'MemberOperatorDetail', params: {idx: item.uid}, not_query: true})
		}
		,clear_item: function(){
			this.item_new = {
				MTOKEN: this.TOKEN
